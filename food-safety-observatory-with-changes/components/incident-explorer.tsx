...
export function IncidentExplorer({ articles }: { articles: ArticleSummary[] }) {
  ...
  return <div className="section-shell py-10"><div className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--muted)]" /><input className="focus-ring h-14 w-full rounded-[4px] border border-[var(--line)] bg-[var(--white)] pl-12 pr-4 text-base" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search titles, sources and full article text" aria-label="Search article repository" /></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><Button variant="secondary" className="lg:hidden" onClick={() => setFiltersOpen(!filtersOpen)}><SlidersHorizontal className="h-4 w-4" />Filters</Button><p className="text-sm text-[var(--muted)]"><strong className="text-[var(--ink)]">{formatNumber(filtered.length)}</strong> results</p><div className="flex items-center gap-2"><Select value={sort} onValueChange={(value) => { setSort(value); setPage(1); }}><SelectTrigger className="h-10 w-44" aria-label="Sort results"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="newest">Newest first</SelectItem><SelectItem value="oldest">Oldest first</SelectItem><SelectItem value="confidence">Highest confidence</SelectItem><SelectItem value="title">Title A-Z</SelectItem></SelectContent></Select><div className="flex border border-[var(--line)] bg-[var(--white)] p-0.5"><button className={cn("focus-ring grid h-9 w-9 place-items-center", view === "grid" && "bg-[var(--maroon)] text-white")} onClick={() => setView("grid")} title="Grid view" aria-label="Grid view"><Grid2X2 className="h-4 w-4" /></button><button className={cn("focus-ring grid h-9 w-9 place-items-center", view === "list" && "bg-[var(--maroon)] text-white")} onClick={() => setView("list")} title="List view" aria-label="List view"><List className="h-4 w-4" /></button></div></div></div>
